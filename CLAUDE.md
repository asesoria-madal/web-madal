...
**Rediseño visual v2 — 2026-09-17 (implementado en código, sin commitear todavía):** nueva ronda de diseño, empezada de cero, inspirada en 4 referencias externas (8vo Designs, Waaark, Born Group, Root Studio) pero manteniendo intacta la paleta/tipografía de arriba. Canvas con el historial de iteración: https://claude.ai/artifact/KZBoezyiPCYywwpAqY21Uo. El sistema final quedó aplicado directamente en `Home.astro`/`global.css`/`Header.astro` (hero partido asimétrico, CTA circular hover + botón `.btn-stamp` con volumen, ticket de precio, imán en botones, más color en "La diferencia"/tabla de precios/"Cómo empezamos", fix del bug del desplegable de servicios, hamburguesa a 1600px, `TrustBanner.astro` con frases reales). Sigue pendiente sin definir la sección de "prueba real" (balance/PyG anonimizado) ya mencionada abajo, y el banner de confianza sigue sin logos reales (candidatos: CEC, Punt PAE, Colaborador Social AEAT — ver comentario en `TrustBanner.astro`).
