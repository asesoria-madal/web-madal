...
Hubo una ronda de exploración visual de la home (hero tipográfico, tabla comparativa estilo Bauhaus, etc.) iterada en un canvas de diseño publicado como Artifact: https://claude.ai/code/artifact/2487eeb5-2f05-4653-b05d-472e741df4f4 — el resultado de esa exploración ya se aplicó directamente al código (ver más abajo), el canvas queda solo como referencia histórica.

**Rediseño visual v2 — 2026-09-17 (en producción):** ronda de diseño empezada de cero, inspirada en 4 referencias externas (8vo Designs, Waaark, Born Group, Root Studio) pero manteniendo intacta la paleta/tipografía de arriba. Canvas con el historial de iteración: https://claude.ai/artifact/KZBoezyiPCYywwpAqY21Uo. Sistema final en `Home.astro`/`global.css`/`Header.astro`: hero partido asimétrico con ticket de precio (colores de modo claro fijos incluso en oscuro, a propósito), CTA circular hover + botón `.btn-stamp` con volumen (tonos intercambiados en modo oscuro), más color en "La diferencia"/tabla de precios/"Cómo empezamos", `TrustBanner.astro` con frases reales (sin logos — candidatos CEC/Punt PAE/Colaborador Social AEAT, ver comentario en el componente). Sigue pendiente sin definir la sección de "prueba real" (balance/PyG anonimizado), ver más abajo.

**Navegación — 2026-09-17 (revertido el mismo día):** se probó la hamburguesa como único punto de entrada también en escritorio, pero se volvió al menú horizontal clásico (`nav.primary`) en escritorio, con la hamburguesa otra vez solo para `max-width: 860px`. El enlace de "Alta" se quitó del nivel superior del menú (ya está dentro del desplegable de "Nuestros servicios"), que ahora lleva una flechita de texto (`▾`, no icono) para indicar que se abre en hover.

Tres gotchas de CSS que costaron tiempo de depurar en esta ronda, por si se repiten: **(1)** el contenido de un `<details>` no se deja forzar a visible con CSS de forma fiable cuando no está `[open]` — usar botón + `div` con clase de estado si hace falta un desplegable siempre-en-el-DOM; **(2)** `backdrop-filter` (como el de `header.site`) crea un contexto de posicionamiento propio para descendientes `position:fixed`, que se quedan encogidos a su altura en vez de llegar a toda la pantalla; **(3)** transiciones de opacity/visibility pueden quedarse a medias combinadas con la regla global `prefers-reduced-motion` de este sitio — mejor `display:none/block` sin transición para mostrar/ocultar por hover/clic.
