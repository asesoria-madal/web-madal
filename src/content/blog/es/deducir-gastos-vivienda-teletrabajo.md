---
lang: es
slug: deducir-gastos-vivienda-teletrabajo
title: '¿Puedes deducirte el alquiler, la luz o el teléfono si trabajas desde casa?'
description: 'Sí, pero solo la parte proporcional — así se calcula.'
cat: Fiscalidad
date: 2026-08-19
---

Si trabajas desde casa, una parte de tus gastos de vivienda son deducibles. La condición es que declares con precisión qué parte de la vivienda usas para tu actividad, y que apliques el cálculo correcto a cada tipo de gasto.

## La condición: afectación parcial

Hacienda exige que declares qué porcentaje de la vivienda destinas a tu actividad — normalmente los metros cuadrados de la habitación o el espacio que usas como despacho o taller, sobre el total de la vivienda. A eso se le llama "afectación parcial", y es el porcentaje base sobre el que se calculan las deducciones.

## Qué puedes deducir y cómo se calcula

- **Alquiler o hipoteca (intereses), IBI, comunidad y seguro de hogar**: proporcional al porcentaje de metros cuadrados afectos a la actividad.
- **Suministros — luz, agua, gas, internet**: aquí la ley aplica un criterio distinto al del alquiler: puedes deducir el 30% de la parte proporcional a los metros afectos, no el 100% de esa proporción. No es el mismo cálculo que para el alquiler, así que conviene que lo ajustemos a tu caso concreto.
- **Teléfono**: deducible si es una línea identificable como profesional — lo más limpio es tener una línea separada de la personal.

## Lo que hace falta para que no te lo discutan

Declarar el porcentaje de afectación en el modelo censal correspondiente, guardar todas las facturas de suministros y vivienda a tu nombre, y que el porcentaje declarado sea razonable respecto al uso real del espacio.

## Por qué conviene calcularlo bien, no a ojo

Un porcentaje muy alto puede levantar preguntas de Hacienda; uno muy bajo simplemente te deja dinero deducible sin aprovechar. Lo revisamos contigo con los datos reales de tu vivienda — y si quieres ver el resto de gastos que puedes deducir como autónomo, lo tienes todo en [qué gastos te puedes deducir](/blog/gastos-deducibles-autonomos).
